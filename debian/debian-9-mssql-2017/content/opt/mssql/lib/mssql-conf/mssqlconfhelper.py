...
def printEngineWelcomeMessage():
    ...
    print ("")
    print ("+--------------------------------------------------------------+")
    print (_("Please run '/opt/mssql/bin/mssql-conf setup'"))
...
def checkSudo():
    ...
    # Hack to bypass root check so it can be
    # run as mssql user only
    # Default: False
    return True
...
def printStartSqlServerMessage():
    ...
    print(_("Please run 'systemctl start mssql-server' to start SQL Server."))
...
def checkInstall():
    ...
    # Hack to bypass root check so it can be
    # run as mssql user only
    # Default: runScript(checkInstallScript, True) == 0
    return True
...
def checkRunningInstance():
    ...
    if (ret == 0):
        ...
        print ("")
        print ("    systemctl stop mssql-server")
        return True
...
def setupSqlServer(eulaAccepted, noprompt=False):
    ...
    #
    # Container hack bypass systemctl / service
    # ret = subprocess.call(["sudo", "systemctl", "start", "mssql-server"])
    # if (ret != 0):
